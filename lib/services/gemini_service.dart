...
class GeminiService {
  ...
  static String get baseUrl =>
      'https://generativelanguage.googleapis.com/v1beta/models/$_primaryModel:generateContent';
  ...
  GeminiService({DatabaseHelper? dbHelper})
  ...
  Future<Map<String, dynamic>> getMemoryContext() async {
    try {
      developer.log(
        '[GeminiService] 🔍 Loading memory context from database...',
      );
      ...
      final allRoutines = await _dbHelper.getDailyRoutines(
        _currentUserId ?? '',
      );
      ...
      final allReminders = await _dbHelper
          .getAllReminders(); // Note: reminders table might not be userId-aware yet in stub
      ...
      final safetyLocations = await _dbHelper.getSafetyLocations(
        _currentUserId ?? '',
      );
      final zones = safetyLocations
          .map(
            (s) => {
              'name': s.name,
              'radius': s.radius,
      ...
      final todayRoutines = allRoutines
          .where((r) => r.isActive && r.days.contains(todayWeekday))
          .toList();
      ...
      developer.log(
        '[GeminiService] ✅ Loaded context: ${journals.length} journals, ${activeMeds.length} meds, ${todayRoutines.length} routines today',
      );
      ...
      return {
        ...
        'todayAgenda': {
          ...
          'reminders': todayReminders
              .map(
                (r) =>
                    '${DateFormat('h:mm a').format(r['dateTime'])}: ${r['title']}',
              )
              .toList(),
        },
      };
  ...
  Future<bool> validateApiKey() async {
    ...
    try {
      ...
      developer.log(
        '[GeminiService] Validation response: ${response.statusCode}',
      );
  ...
  Future<http.Response> _makeApiRequestWithRetry({
    ...
    while (attempt < maxRetries) {
      try {
        developer.log(
          '[GeminiService] API request attempt ${attempt + 1}/$maxRetries',
        );
        ...
        if (response.statusCode == 503 || // Service Unavailable
          ...
          developer.log(
            '[GeminiService] ⚠️ Retryable error ${response.statusCode}: $errorMsg',
          );

          if (attempt < maxRetries - 1) {
            final delay = _getRetryDelay(attempt);
            developer.log(
              '[GeminiService] ⏳ Retrying in ${delay.inSeconds}s...',
            );
        ...
        if (response.statusCode == 401 || response.statusCode == 403) {
          developer.log(
            '[GeminiService] ❌ Authentication error - not retrying',
          );
          return response;
        }
        ...
        developer.log(
          '[GeminiService] 🌐 Network error on attempt ${attempt + 1}: $e',
        );
        ...
        if (attempt < maxRetries - 1) {
          ...
          developer.log(
            '[GeminiService] ⏳ Network issue - retrying in ${delay.inSeconds}s...',
          );
          await Future.delayed(delay);
          attempt++;
        } else {
        ...
        lastException = e;
        developer.log(
          '[GeminiService] ❌ Exception on attempt ${attempt + 1}: $e',
        );

        if (attempt < maxRetries - 1) {
          final delay = _getRetryDelay(attempt);
          developer.log(
            '[GeminiService] ⏳ Retrying after exception in ${delay.inSeconds}s...',
          );
          await Future.delayed(delay);
          attempt++;
        } else {
  ...
  Future<void> _switchToFallbackModel() async {
    if (_currentModel != _fallbackModel) {
      developer.log(
        '[GeminiService] 🔄 Switching to fallback model: $_fallbackModel',
      );
  ...
  Future<ChatMessage> sendMessage(
    ...
    if (!hasApiKey) {
      ...
      throw Exception(
        'API key not set. Please configure your Gemini API key in Settings.',
      );
    }
    ...
    try {
      ...
      developer.log(
        '[GeminiService] Loaded ${history.length} recent messages from history',
      );
      ...
      contents.add({
        ...
        'parts': [
          {
            'text':
                'I understand perfectly. I am Menta, your dedicated memory companion. I have reviewed your journals, medications, daily routines, and familiar faces. I will use this information to provide you with compassionate, personalized support while monitoring your safety and well-being. How can I help you today?',
          },
        ],
      });
      ...
      if (imagePath != null) {
        ...
        if (base64Image != null) {
          ...
          imageInstructions.writeln(
            '2. Compare any people in the image against the FAMILIAR FACES list provided above.',
          );
          imageInstructions.writeln(
            '3. If you find a match, greet them warmly and remind the user of their relationship.',
          );
          imageInstructions.writeln(
            '4. If no match is found but there is a person, describe them politely.',
          );
          imageInstructions.writeln(
            '5. If the image is not of a person, describe what you see in the context of memory care.',
          );
      ...
      try {
        ...
        if (_currentModel != _fallbackModel) {
          developer.log(
            '[GeminiService] 🔄 Primary model failed, trying fallback model...',
          );
      ...
      developer.log(
        '[GeminiService] Final response status: ${response.statusCode}',
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data['candidates'] != null && data['candidates'].isNotEmpty) {
          ...
          developer.log(
            '[GeminiService] ========== SEND MESSAGE SUCCESS ==========',
          );
          return aiMessage;
        } else {
        ...
        developer.log(
          '[GeminiService] ERROR: Bad status code ${response.statusCode}',
        );
        ...
        if (response.statusCode == 503) {
          throw Exception(
            'The AI service is temporarily busy. This usually resolves quickly. Please try again in a moment.',
          );
        }
      ...
      developer.log(
        '[GeminiService] ========== SEND MESSAGE FAILED (NETWORK) ==========',
      );
      developer.log('[GeminiService] Network exception: $e');
      throw Exception(
        'Cannot connect to Google AI services. Please check your internet connection and try again.',
      );
    } catch (e) {
      developer.log(
        '[GeminiService] ========== SEND MESSAGE FAILED ==========',
      );
