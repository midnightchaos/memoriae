...
class MentaGamesService extends ChangeNotifier {
  ...
  Future<ChatMessage?> generateGameInvite() async {
    ...
    List<MentaGameType> possibleGames = [MentaGameType.wordAssociation]; // Always possible
    ...
    switch (_activeGameType!) {
      case MentaGameType.faceRecall:
        ...
        content = "I found a photo of someone special. Do you remember who this is?";
        metadata['faceId'] = face.id;
        metadata['imagePath'] = face.photoPath;
        metadata['options'] = _generateOptions(face.name, faces.map((f) => f.name).toList());
        break;
      ...
      case MentaGameType.journalQuiz:
        ...
        content = "You wrote something wonderful recently. Do you remember what you titled this entry: '${journal.content.substring(0, min(50, journal.content.length))}...'?";
        metadata['journalId'] = journal.id;
        metadata['options'] = _generateOptions(journal.title, journals.map((j) => j.title).toList());
        break;
      ...
      case MentaGameType.routineCheck:
        ...
        content = "It's almost ${routine.time}. Do you remember what we usually do at this time?";
        metadata['routineId'] = routine.id;
        metadata['options'] = _generateOptions(routine.title, routines.map((r) => r.title).toList());
        break;

      case MentaGameType.wordAssociation:
        final words = ["Garden", "Library", "Ocean", "Mountain", "Kitchen", "Piano"];
        final word = words[_random.nextInt(words.length)];
        _activeGameData = word;
        _correctAnswer = null; // Open ended
        content = "Let's play Word Association! I'll say a word, and you tell me the first thing that comes to mind. The word is: **$word**";
        metadata['word'] = word;
        break;
    }
  ...
  Future<ChatMessage> handleResponse(String response) async {
    ...
    if (_activeGameType == MentaGameType.wordAssociation) {
      isCorrect = true;
      feedback = "That's a wonderful association! Memory is like a beautiful tapestry, isn't it?";
    ...
    if (userId != null && _activeGameType != null) {
      ...
      await _db.saveGameProgress(GameProgress(
        id: DateTime.now().millisecondsSinceEpoch.toString(),
        userId: userId,
